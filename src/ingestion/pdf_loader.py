...
def load_and_process_pdfs(pdf_folder_path: str, config: dict) -> list[Document]:
    """
    Loads and processes PDFs using the 'unstructured' library.
    Image processing is disabled for deployment reliability.
    """
    documents = []
    ingestion_config = config.get('ingestion', {})
    strategy = ingestion_config.get('parsing_strategy', 'fast')
    # Image processing is now controlled by config and defaults to False
    process_images_flag = ingestion_config.get('process_images', False)
    ...
    for file in os.listdir(pdf_folder_path):
        ...
        elements = partition_pdf(
            ...
            strategy=strategy,
            infer_table_structure=True,
            # We set this to False to avoid external API calls and dependencies
        ...
        if page_content:
            documents.append(Document(
                page_content=page_content,
                metadata={'source': file, 'page': element.metadata.page_number if hasattr(element, 'metadata') else 0}
            ))
