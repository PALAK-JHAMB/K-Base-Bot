...
def load_all_resources():
    ...
    # --- Load RAG Chain ---
    try:
        # Pass the retriever object directly when creating the chain
...
if prompt := st.chat_input("Ask your question..."):
    ...
    # All logic to generate and display the response now happens inside this block
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            ...
            # 1. First, try to find an answer in the FAQ (Excel)
            faq_answer = get_faq_answer(prompt, faq_data)
            
            if faq_answer:
                response = f"**From FAQ:**\n\n{faq_answer}"
            # else:
            #     # 2. If not in FAQ, use the RAG chain
            #     st.info("No FAQ match found. Searching documents...")
                
            #     # The chain expects a dictionary with a "question" key
            #     result = rag_chain.invoke({"question": prompt})
                
            #     # The output is now a direct string from our updated chain
            #     response = result

            # # Display the final response
            # st.markdown(response)
            # in src/ui/app.py, inside the 'else' block for the RAG pipeline
            else:
                # 2. If not in FAQ, use the RAG chain
                st.info("No FAQ match found. Searching documents...")
    
                 # This new chain just needs the prompt string.
                response = rag_chain.invoke(prompt)
    
                # The output is a direct string.
                st.markdown(response)
