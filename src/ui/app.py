...
#         # This builder uses Hugging Face embeddings and needs no API key
...
#         # This uses the free, local Hugging Face model
...
def load_all_resources():
    ...
    try:
        ...
        config = {
            ...
            "ingestion": {"parsing_strategy": "fast", "process_images": False},
            "huggingface": {"llm_repo_id": "google/flan-t5-small"}
        }

    # --- 2. Build Vector Store if not exists ---
    vector_store_path = os.path.join(PROJECT_ROOT, config['data']['vector_store_path'])
    if not os.path.exists(vector_store_path):
        st.info("Knowledge base not found. Building it now. This may take a few minutes...")
        build_vector_store(config)
    ...
    retriever = None
    try:
        embeddings = HuggingFaceEmbeddings(model_name=config['huggingface']['embedding_model_name'])
        vector_store = FAISS.load_local(
            vector_store_path,
            embeddings,
            allow_dangerous_deserialization=True
        ...
        print(f"Retriever Loaded: FAILED → {e}")

    # --- 4. Load RAG Chain ---
    rag_chain = None
    try:
        rag_chain = get_rag_chain(retriever, config)
        print(f"RAG Chain Loaded: SUCCESS")
    except Exception as e:
        print(f"RAG Chain Loaded: FAILED → {e}")

    if retriever is None or rag_chain is None:
        st.error("Failed to load the RAG pipeline. Please check logs.")
        st.stop()

    print("--- ALL RESOURCES LOADED SUCCESSFULLY ---\n")
    return None, retriever, rag_chain
...
if prompt := st.chat_input("Ask your question..."):
    ...
    # Cap chat history to last 10 messages
